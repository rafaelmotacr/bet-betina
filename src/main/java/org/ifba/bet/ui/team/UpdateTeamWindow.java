...
public class UpdateTeamWindow extends JInternalFrame {
	...
	UpdateTeamWindow() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!newName.equals(team.getName())) {
					try {
						...
						JOptionPane.showMessageDialog(UpdateTeamWindow.this,
								"Não foi possível atualizar o nome do time.");
					}
				} else {
				...
				if (!newAbbreviation.equals(team.getAbbreviation())) {
					try {
						...
						teamMainWindow.updateTeams();
						JOptionPane.showMessageDialog(UpdateTeamWindow.this,
								"Abreviação do time atualizado com sucesso.");
					} catch (SQLException e1) {
						JOptionPane.showMessageDialog(UpdateTeamWindow.this,
								"Não foi possível atualizar a abreviação do time.");
					}
				} else {
		...
		bgImageLBL.setIcon(new ImageIcon("src/main/resources/updateTeamBG.png"));
